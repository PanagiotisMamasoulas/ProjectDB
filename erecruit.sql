...
DECLARE appPersonality float(2,1);
DECLARE finishedFlag INT;

DECLARE appCursor CURSOR FOR
select s.personality from applies p inner join job l on l.id = p.job_id left join (select personality,id,cand_username from applies inner join interview on applies.cand_usrname = interview.cand_username inner join job as in_job on in_job.id = applies.job_id where in_job.id = jobId AND interview.recruiter = in_job.recruiter) s on s.cand_username = p.cand_usrname where l.id = jobId;


DECLARE CONTINUE HANDLER FOR NOT FOUND SET finishedFlag=1;
OPEN appCursor;
SET finishedFlag=0;
FETCH next from appCursor INTO appPersonality;
WHILE (finishedFlag=0) DO 
IF (appPersonality is NULL) THEN
SELECT 'Applications pending to be reviewed';
leave label;
END IF; 
...
select t1.cand_usrname,t2.pa as Personality,COALESCE(t3.ga,0) as Grade, COALESCE(t4.pc,0) as Experiance,(t2.pa + COALESCE(t3.ga,0) + COALESCE(t4.pc,0)) as Total from applies t1 left join (select cand_username,round(AVG(personality),1) as pa,count(*) from interview group by cand_username) t2 on t1.cand_usrname = t2.cand_username left join (select cand_usrname,SUM(GRA(bathmida,grade)) as ga from has_degree left join degree on degree.titlos = has_degree.degr_title group by cand_usrname) t3 on t3.cand_usrname = t1.cand_usrname left join (select candid,count(*) as pc from project group by candid) t4 on t4.candid = t1.cand_usrname where t1.job_id = jobId AND t2.pa != 0 AND COALESCE(t3.ga,0) != 0 AND COALESCE(t4.pc,0) != 0 order by (t2.pa + COALESCE(t3.ga,0) + COALESCE(t4.pc,0)) DESC;

create table temp as select t1.cand_usrname,t2.pa as Personality,COALESCE(t3.ga,0) as Grade, COALESCE(t4.pc,0) as Experiance from applies t1 left join (select cand_username,round(AVG(personality),1) as pa,count(*) from interview group by cand_username) t2 on t1.cand_usrname = t2.cand_username left join (select cand_usrname,SUM(GRA(bathmida,grade)) as ga from has_degree left join degree on degree.titlos = has_degree.degr_title group by cand_usrname) t3 on t3.cand_usrname = t1.cand_usrname left join (select candid,count(*) as pc from project group by candid) t4 on t4.candid = t1.cand_usrname where t1.job_id = jobId AND (t2.pa = 0 OR COALESCE(t3.ga,0) = 0 OR COALESCE(t4.pc,0) = 0);


call jobApplicationsFail(jobId);
Drop table if exists temp;
End$
DELIMITER ; 

DELIMITER $
DROP PROCEDURE IF EXISTS jobApplicationsFail$ 
CREATE PROCEDURE jobApplicationsFail(IN jobId INT)
Begin
DECLARE username varchar(12);
DECLARE personality float(2,1);
DECLARE grade float(2.1);
DECLARE experiance INT;
DECLARE message varchar(100);
DECLARE finishedFlag INT;
DECLARE failCursor CURSOR FOR
select * from temp;

open failCursor;
SET finishedFlag=0;
fetch failCursor into username,personality,grade,experiance;
WHILE (finishedFlag=0) DO
set message = '';
...
/*
select * from applies inner join interview on applies.cand_usrname = interview.cand_username inner join job on job.id = applies.job_id where job.id = 2 AND interview.recruiter = job.recruiter;
select p.cand_usrname,s.personality,l.id from applies p inner join job l on l.id = p.job_id left join (select personality,id,cand_username from applies inner join interview on applies.cand_usrname = interview.cand_username inner join job as in_job on in_job.id = applies.job_id where in_job.id = 2 AND interview.recruiter = in_job.recruiter) s on s.cand_username = p.cand_usrname having l.id = 2;


select cand_username,round(AVG(personality),2),count(*) from interview group by cand_username;

select cand_usrname,SUM(GRA(bathmida,grade)) from has_degree left join degree on degree.titlos = has_degree.degr_title group by cand_usrname;

select candid,count(*) from project group by candid;

select t1.cand_usrname,t1.job_id,t2.pa as Personality,COALESCE(t3.ga,0) as Grade, COALESCE(t4.pc,0) as Experiance,(t2.pa + COALESCE(t3.ga,0) + COALESCE(t4.pc,0)) as Total from applies t1 left join (select cand_username,round(AVG(personality),1) as pa,count(*) from interview group by cand_username) t2 on t1.cand_usrname = t2.cand_username left join (select cand_usrname,SUM(GRA(bathmida,grade)) as ga from has_degree left join degree on degree.titlos = has_degree.degr_title group by cand_usrname) t3 on t3.cand_usrname = t1.cand_usrname left join (select candid,count(*) as pc from project group by candid) t4 on t4.candid = t1.cand_usrname order by t1.job_id;
...
/*
DROP TRIGGER IF EXISTS insUserB;
DELIMITER $
CREATE TRIGGER insUserB
Before insert on user
FOR EACH ROW
BEGIN
if(new.username = null) then
Insert into history (user,status,type,table_name) values
(@curUser,'Not Success','Insert','user');
END IF;
END$
DELIMITER ;

DROP TRIGGER IF EXISTS insUserA;
DELIMITER $
CREATE TRIGGER insUserA
After insert on user
FOR EACH ROW
BEGIN
Declare max_date datetime;
select max(date) into max_date from history;
Update history set status = 'Success' where date = max_date;
